...
3. defer 脚本会在 DOMContentLoaded 事件之前执行，而 async 脚本则不一定。
...
层叠上下文（Stacking Context）是一个抽象概念，它定义了一个元素在z轴上的层次。在同一个层叠上下文中，元素的堆叠顺序由`z-index`属性控制。层叠上下文可以嵌套，形成一个层叠上下文树。
...
闭包是指一个函数可以访问另一个函数作用域内的变量。在JavaScript中，当一个函数嵌套在另一个函数中时，内部函数可以访问外部函数的变量，即使外部函数已经返回了。这种情况下，内部函数形成了一个闭包，它保留了外部函数的作用域链并可以继续访问这些变量。闭包常常用于实现函数的封装和私有化，以及在回调和事件处理等场景下的数据共享与传递。
...
8垃圾回收是对JavaScript垃圾回收策略的具体实现。它采用分代收集和增量标记两种算法，有效降低了垃圾回收对程序性能的影响。
...
Promise 是 JavaScript 中用于处理异步操作的一种编程模式。它提供了一种更简洁、更易于理解的方法来处理异步操作的结果和错误，而不是使用回调函数。以下是 Promise 的一些特性、优点和缺点：

特性：

1. 状态：Promise 有三种状态：pending（待定）、fulfilled（已实现）和rejected（已拒绝）。Promise 在创建时处于 pending 状态，然后根据异步操作的结果变为 fulfilled 或 rejected。状态一旦改变，就无法再次更改。
2. 链式调用：Promise 支持链式调用，这意味着可以将多个异步操作连接在一起，以有序的方式处理它们的结果。通过使用 `.then()` 和 `.catch()` 方法，可以简洁地处理成功和失败的情况。

优点：

1. 错误处理：Promise 提供了一种更好的错误处理机制，可以使用 `.catch()` 方法集中处理错误，而不是在每个回调函数中处理。
2. 避免回调地狱：Promise 通过链式调用简化了异步代码的结构，避免了回调地狱的问题。这使得代码更容易阅读和维护。
3. 同时处理多个异步操作：使用 `Promise.all()` 和 `Promise.race()` 方法，可以轻松地处理多个异步操作。`Promise.all()` 等待所有操作完成，`Promise.race()` 返回最先完成的操作的结果。
...
尽管存在一些缺点，Promise 仍然是 JavaScript 中处理异步操作的一个强大工具。它们提供了一种更可读、更易于维护的方式来编写异步代码。不过，在某些情况下，可能需要考虑其他异步处理方法，如 async/await（基于 Promise）或者其他异步编程库。
...
3. 作用域：ESM 和 CommonJS 在处理变量作用域方面有所不同。
   ...
   - CommonJS 使用文件作用域，但与 ESM 不同，CommonJS 模块可以通过 `global` 对象访问全局作用域。
...
c) beforeMount：在模板编译完成、挂载 DOM 之前触发。此时，虚拟 DOM 尚未创建，真实 DOM 尚未更新。

d) mounted：在模板编译完成、挂载 DOM 之后触发。此时，虚拟 DOM 已经创建，真实 DOM 已经更新。
