...
f) updated：在数据发生变化，组件重新渲染并更新 DOM 之 后触发。此时，可以获取到新的 DOM 结构。
...
React Fiber 是React 16版本中引入的一种新的核心算法，主要用于改善React的渲染过程。"Fiber"实际上指的是一种可以管理工作单元的数据结构。React通过Fiber来控制渲染的优先级，实现任务的可中断和可恢复，从而改善应用的响应性能。

以下是我对Fiber的一些理解：

1. **渲染的可中断性**：在早期的React版本中，一旦开始渲染组件，就不能被中断，必须等到渲染完成后，才能进行其他操作。这在处理大型应用时可能会引发性能问题，因为JavaScript运行在单线程环境中，长时间的渲染任务可能会阻塞用户界面，导致应用无法快速响应用户的操作。Fiber引入了一个新的功能——渲染的可中断性，使得React可以在执行长时间的渲染任务时，根据任务的优先级，将任务划分为多个小任务，并将它们分布到多个帧中去执行，从而提高应用的响应性能。

2. **优先级调度**：Fiber引入了一种基于优先级的调度机制。不同类型的更新（如用户交互、动画等）可以具有不同的优先级。React可以根据这些优先级，确定应该优先完成哪些工作。例如，用户交互和动画通常需要更快的响应，因此可以被赋予较高的优先级。

3. **双缓冲技术**：Fiber实现了一种类似于计算机图形渲染中的双缓冲（double buffering）的技术，它维护了两棵Fiber树（当前的Fiber树和工作中的Fiber树），以避免在渲染过程中出现不一致的UI。

4. **更好的错误边界处理**：通过Fiber，React 16也引入了一种新的生命周期方法——`getDerivedStateFromError`和`componentDidCatch`，这两个方法用于捕获子树级别的JavaScript错误，并允许我们在UI中优雅地处理这些错误。

总的来说，React Fiber是对React核心算法的一次重大改进，它使得React在处理大型和复杂应用时，具有更好的性能和更强的可控性。
